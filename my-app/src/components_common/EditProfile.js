...
const EditProfile = ({ setEditProfileBtn }) => {
  ...
  useEffect(() => {
    const fetchUserData = async () => {
      try {
        const response = await fetch('http://ec2-54-153-39-198.us-west-1.compute.amazonaws.com:5000/profile_data', {
          method: 'POST',
  ...
  const handleSubmit = async () => {
    try {
      ...
      const response = await fetch('http://ec2-54-153-39-198.us-west-1.compute.amazonaws.com:5000/edit_profile_data', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          user_id: user_id,
