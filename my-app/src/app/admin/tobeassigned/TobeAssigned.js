...
function TobeAssigned() {
  ...
  const fetchCourses = async () => {
    try {
      const response = await fetch('ec2-54-183-22-164.us-west-1.compute.amazonaws.com:5000/get_courses_tobe_assigned', {
          method: 'POST',
  ...
  const fetchFaculty = async () => {
    try {
      const response = await fetch('ec2-54-183-22-164.us-west-1.compute.amazonaws.com:5000/get_faculty_user_ids', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({}) 
      });
  ...
  const assignCourse = async (courseId) => {
    ...
    try {
      await fetch('ec2-54-183-22-164.us-west-1.compute.amazonaws.com:5000/assign_course', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
