...
function ToGrade() {
    ...
    useEffect(() => {
        async function fetchAssignments() {
            try {
                const response = await fetch('https://ec2-54-67-80-169.us-west-1.compute.amazonaws.com:3000:5000/get_tograde_assignments', {
                    method: 'POST',
    ...
    const handleGradeAssignment = async (studentId, assignmentId, totalPoints, event) => {
        ...
        try {
            const response = await fetch('https://ec2-54-67-80-169.us-west-1.compute.amazonaws.com:3000:5000/assign_grade', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({
